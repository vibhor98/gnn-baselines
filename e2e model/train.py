...
# export LD_LIBRARY_PATH="/home/rohit/miniconda3/lib:$LD_LIBRARY_PATH"
...
def main(args):
    ...
    data, labels = dgl.load_graphs('guest_dgl_graphs_ids.dgl')
    print('No. of Graphs:', len(data))
    num_examples = int(len(data) * 0.6)
    num_train = int(num_examples * 0.4) #0.8
    ...
    for batched_graph in test_dataloader:
        ...
        logits = model(batched_graph, batched_graph.ndata['node_id'])
        pred = logits.argmax(1)
        pos_probs.extend(logits[:, 1].cpu().detach().numpy())
